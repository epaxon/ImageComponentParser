function [segment_masks, segment_info] = segment_ics(comp, x, y)
...
% Ok bunch of different magic numbers
abs_thresh = 1.2;
min_area = 6;
resize_scale = 1/4;
...
c = 1;
for i = 1:size(comp, 3)
    filt_im = imresize(imresize(comp(:,:,i), resize_scale), size(comp(:,:,i)));
    
    pos_mask = filt_im > abs_thresh;
    neg_mask = -filt_im > abs_thresh;
